...
tibble(x =1:5 , y = 1 , z = as.integer(x ^ 2 + y))
...
iris_raw %>% 
  ...
  summarise(sepal_length_mean = mean(sepal_length), 
             sepal_lenght_median = median(sepal_length),
...
named_vector <- iris_raw %>% 
  ...
  filter(sepal_length < 6) %>%
  arrange(desc(sepal_length)) %>%
  pull(sepal_lenght, name=species)
...
theme(axis.text.x=element_text(size=rel(2), hjust=1, vjust=1, colour="black"),
   ...
   labs(x= "Sepal area", y="Observations", fill="Species") 
...
ggplot(iris_for_ggplot, aes(x=sepal_area, fill=species)) +
geom_histogram()
theme(axis.text.x=element_text(size=rel(1), hjust=1, vjust=1, colour="black"),
...
> Answer:
We lost which petal_length and width are together
...
iris_long2 <- iris_raw %>% 
  ...
  select(!c(ID,species))%>%
   pivot_longer(!c(species_ID), names_to = "variable", values_to="value")
...
exprs_assay <- read_xlsx("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet="exprs", range=NULL)
...
ggplot(exprs_assay_long, aes(x=expression, fill=gene))+
  geom_histogram()+
  facet_wrap(~gene)+
  labs(x= "Expression", y="Observations", fill="Gene")
...
```{r}
excel_sheets("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx")
sample_info <- read_xlsx("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet = "sample_info")
...
table_for_ggplot <- exprs_sampleinfo %>% 
  ggplot(exprs_sampleinfo, aes(x=grade))+
  geom_histogram()+
