...
# shows the meaning of each letter
AA_ALPHABET
...
# Bigger the value, higher the quality
# So should aim for 35 or higher
# lower case letters are the ones with the highest quality
```
...
```{r}
file.edit("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")

readDNAStringSet("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa", format="fasta")
truseq_adapters <- readDNAStringSet("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa", format="fasta")
truseq_adapters
```
...
colnames(mcols(gtf_ACTB))
```

- How many transcripts do the annotations describe for that gene?

```{r}
unique_transcript <- unique(gtf_ACTB$transcript_id)
unique_transcript <- na.omit(unique_transcript)
length(unique_transcript)
...
```{r}
human_symbols <- keys(org.Hs.eg.db, "SYMBOL")[1:6]
human_symbols <- head(keys(org.Hs.eg.db, "SYMBOL"))  
human_symbols
```
...
```{r}
annotation_dbi <- AnnotationDbi::select(org.Hs.eg.db, human_symbols, c("ENTREZID", "ENSEMBL"), "SYMBOL")


select(org.Hs.eg.db, human_symbols, c("ENTREZID", "ENSEMBL"), "SYMBOL")
```
...
mapIds is wrapping the data as a vector
...
# list of datasets available today
```
...
```{r}
mart <- useMart(biomart="ENSEMBL_MART_ENSEMBL", host= "https://may2021.archive.ensembl.org") #connect to biomart
mart
```
...
```{r}
list_dataset <- listDatasets(mart); 
list_dataset
```
...
```{r}
mart <- useMart("ENSEMBL_MART_ENSEMBL", "hsapiens_gene_ensembl", host="https://may2021.archive.ensembl.org"
...
```{r}
assays(demo_se) #name of assay is counts and it is in a list
assayNames(demo_se) #name of assay is counts
assay(demo_se, "counts") #from the overal object, give me an assay named counts and shows the matrix
colData(demo_se) #dataframe that has been created to show the columns
rowData(demo_se) #dataframe that has been created to show the rows
mcols(demo_se) #same as rowData?
metadata(demo_se) #?
```
...
assay_counts <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv")


rownames(assay_counts) <- assay_counts$X 
assay_counts$X <- NULL #deletes a column
assay_counts <- as.matrix(assay_counts)
head(assay_counts)

# shift + command + enter = runs the whole chunk
```
...
```{r}
sample_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv", row.names=1) #row.names=1 transforms the first column into the row names
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv", row.names=1)
...
gene_metadata, sample_metadata, assay_counts
```{r}
demo_se <- SummarizedExperiment(
