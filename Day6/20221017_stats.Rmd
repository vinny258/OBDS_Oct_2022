...
rnorm = normally distributed data
...
Goes from 0 to 1 in 0.1 steps

```{r}
y <- quantile(x, probs = seq(0, 1, 0.1)); y
```

- Visualise the distribution of the x values as a histogram.
...
abline(v=(mean(x)-sd(x)), col="blue", lty=1)
abline(v=(mean(x)+sd(x)), col="blue", lty=1)
...
```{r}
set.seed(1)
m <- rnorm(n=1000000, mean=10, sd=5)
hist(m, breaks=100, col="grey")
...
norm_dat <- data.frame(x=a, cdf=pnorm(a))
ggplot(norm_dat) + 
...
norm_dat <- data.frame(x=a, cdf=qnorm(a))
ggplot(norm_dat) + 
  geom_point(aes(x=a, y=cdf))+
  geom_line(aes(x=a, y=cdf))
...
```{r}
a <- seq(from=-5, to=5, by=0.1)
norm_dat <- data.frame(x=a, cdf=dnorm(a))
ggplot(norm_dat) + 
  geom_point(aes(x=a, y=cdf))+
  geom_line(aes(x=a, y=cdf))
...
```{r}
probgreater2 <- 1-pnorm(2); probgreater2
...
```{r}
probminus2to2 <- pnorm(2) - pnorm(-2); probminus2to2
...
probextremes <- 1-(pnorm(2) - pnorm(-2)); probextremes
```
```{r}
library(readxl)
gene_exp <- excel_sheets("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx")
gene_exp
```
...
library(readxl)
excel <- read_excel("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", "logcounts")
gene_exp <- excel_sheets("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx")
head(excel)
excel <- column_to_rownames(excel, var= "gene")
excel <- as.matrix(excel)
class(excel)
...
cell_info <- read_excel("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", "cell_info")
head(cell_info)

gene_exp <- excel[1,]; gene_exp 


infection <- data.frame(group=cell_info$Infection, gene_exp)
infection
t_test <- t.test(gene_exp ~ group, paired = FALSE, data = infection)
t_test
...
infection <- data.frame(group=cell_info$Infection, gene_exp)
t_test <- t.test(gene_exp ~ group, paired = FALSE, data = infection)
p_value <- t_test$p.value; p_value
...
t_test_row <- function(index, metadata, gene_expression_data) {

gene_exp <- gene_expression_data[index,]
infection <- data.frame(group=metadata$Infection, gene_exp)
t_test <- t.test(gene_exp ~ group, paired = FALSE, data = infection)
p_value <- t_test$p.value
return(p_value)
}
...
t_test_row(index = 2, metadata = cell_info, gene_expression_data = excel)
...
t_test_row(index = 5, metadata = cell_info, gene_expression_data = excel)
```
